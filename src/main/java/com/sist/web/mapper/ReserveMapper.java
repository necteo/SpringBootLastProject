...
public interface ReserveMapper {
	...
	@Select("SELECT r.no, cno, rday, rtime, rinwon, "
		  + "TO_CHAR(regdate, 'YYYY-MM-DD') dbday, isReserve, isCancel, title, image1, address "
	...
	// vo.getSvo().setTitle()
	@Select("SELECT r.no, id, cno, rday, rtime, rinwon, "
		  + "TO_CHAR(regdate, 'YYYY-MM-DD') dbday, isReserve, isCancel, title, image1, address "
		  + "FROM reserve_3 r, seoultravel s "
		  + "WHERE r.cno = s.contentid "
		  + "ORDER BY no DESC")
